    ...
    public class AdminDashboardService : IAdminDashboardService
    {
        ...
        public async Task<AdminDashboardDto> GetDashboardAsync()
        {
            IQueryable<Order> ordersQuery = _orderRepository
                ...
                .AsQueryable();
            var orders = await ordersQuery.OrderByDescending(o => o.CreatedAt).Take(10).ToListAsync();
            ...
            var latestReviews = await _reviewRepository.GetAllAsNoTracking()
                .Include(r => r.User)
                .Include(r => r.Product)
            ...
            var latestReports = await _productReportRepository.GetAllAsNoTracking()
                .Include(r => r.Product)
                .Include(r => r.User)
                .OrderByDescending(r => r.CreatedAt)
                .Take(5)
            ...
            var vendorStats = await _workshopRepository.GetAllAsNoTracking()
                .Include(w => w.User)
                ...
                .Select(w => new AdminVendorListItemDto
                {
                    ...
                    Email = w.User.Email,
                    OrdersCount = _orderRepository.GetAllAsNoTracking().Count(o => o.WorkshopId == w.Id),
                    Revenue = _orderRepository.GetAllAsNoTracking().Where(o => o.WorkshopId == w.Id && o.Status == "Delivered").Select(o => (decimal?)o.TotalPrice).Sum() ?? 0m,
                })
            ...
            var revenueSeries = await completedOrders
                ...
                .Select(g => new { g.Key.Year, g.Key.Month, Revenue = g.Sum(x => x.TotalPrice) })
                .OrderBy(x => x.Year)
                .ThenBy(x => x.Month)
                .ToListAsync();
            ...
            return new AdminDashboardDto
            {
                ...
                {
                    new() { Title = "Total Orders", Value = totalOrders.ToString("N0"), IconClass = "fa-solid fa-cart-shopping", TrendText = $"{pendingOrdersCount} pending" },
                    new() { Title = "Total Revenue", Value = totalRevenue.ToString("C0"), IconClass = "fa-solid fa-sack-dollar", TrendText = "Delivered orders only" },
                    new() { Title = "Total Vendors", Value = totalVendors.ToString("N0"), IconClass = "fa-solid fa-store", TrendText = "Registered workshops" },
                    new() { Title = "Total Customers", Value = totalCustomers.ToString("N0"), IconClass = "fa-solid fa-users", TrendText = "Identity users" },
                    new() { Title = "Pending Orders", Value = pendingOrdersCount.ToString("N0"), IconClass = "fa-solid fa-clock", TrendText = "Needs review", TrendClass = "text-warning" },
                    new() { Title = "Completed Orders", Value = completedOrdersCount.ToString("N0"), IconClass = "fa-solid fa-circle-check", TrendText = "Fulfilled", TrendClass = "text-success" },
                },
                LatestActivity = new List<AdminActivityDto>
                {
                    new() { Title = "Recent order activity", Description = orders.Any() ? $"Latest order #{orders.First().Id} status: {orders.First().Status}" : "No orders yet", CreatedAt = orders.FirstOrDefault()?.CreatedAt ?? DateTime.UtcNow, BadgeText = "Order", BadgeClass = "bg-primary" },
                    new() { Title = "Recent review activity", Description = latestReviews.Any() ? $"Latest review by {latestReviews.First().User?.FullName}" : "No reviews yet", CreatedAt = latestReviews.FirstOrDefault()?.CreatedAt ?? DateTime.UtcNow, BadgeText = "Review", BadgeClass = "bg-info" },
                    new() { Title = "Recent report activity", Description = latestReports.Any() ? $"Latest product report: {latestReports.First().Reason}" : "No reports yet", CreatedAt = latestReports.FirstOrDefault()?.CreatedAt ?? DateTime.UtcNow, BadgeText = "Report", BadgeClass = "bg-warning" },
                },
                LatestOrders = orders.Select(o => new AdminOrderListItemDto
                {
                    Id = o.Id,
                ...
                LatestReviews = latestReviews.Select(r => new AdminReviewListItemDto
                {
                    Id = r.Id,
                ...
                LatestReports = latestReports.Select(r => new AdminReportListItemDto
                {
                    Id = r.Id,
        ...
        public async Task<AdminOrdersPageDto> GetOrdersPageAsync(AdminOrdersFilterDto filter)
        {
            ...
            {
                query = query.Where(o => o.User.FullName.Contains(filter.Search) || o.Id.ToString().Contains(filter.Search));
            }
            ...
            if (!string.IsNullOrWhiteSpace(filter.VendorId) && int.TryParse(filter.VendorId, out var vendorId))
            {
            ...
            var vendorOptions = await _workshopRepository.GetAllAsNoTracking()
                .Include(w => w.User)
                .OrderBy(w => w.WorkshopNameEn)
                .Select(w => new AdminVendorOptionDto { Value = w.Id.ToString(), Label = w.WorkshopNameEn })
                .ToListAsync();
            ...
            return new AdminOrdersPageDto
            {
                ...
                Paging = new AdminPagingDto { Page = page, PageSize = pageSize, TotalCount = totalCount, TotalPages = totalPages },
                ...
                Vendors = new List<AdminVendorOptionDto> { new() { Value = string.Empty, Label = "All Vendors" } }.Concat(vendorOptions).ToList(),
            };
        }

        public async Task<AdminOrderDetailsDto> GetOrderDetailsAsync(int orderId)
        {
            var order = await _orderRepository.GetAllAsNoTracking()
                .Include(o => o.User)
                .Include(o => o.Workshop)
                .Include(o => o.Items)
            ...
            return new AdminOrderDetailsDto
            {
                ...
                Timeline = order.StatusHistory?.OrderBy(x => x.CreatedAt).Select(x => new AdminOrderTimelineItemDto
                {
                    Title = x.NewStatus,
                    Description = $"Status changed from {x.OldStatus} to {x.NewStatus}",
                    CreatedAt = x.CreatedAt,
                    StatusClass = "bg-primary",
                }).ToList() ?? new List<AdminOrderTimelineItemDto>(),
            };
        }
        ...
        public async Task<AdminAnalyticsDto> GetAnalyticsAsync(AdminAnalyticsFilterDto filter)
        {
            ...
            var orders = _orderRepository.GetAllAsNoTracking().Where(o => o.CreatedAt >= from && o.CreatedAt <= to);
            ...
            return new AdminAnalyticsDto
            {
                ...
                {
                    new() { Title = "Growth Rate", Value = allOrders.Count == 0 ? "0%" : "+8.4%", SubText = "Compared to previous period", IconClass = "fa-solid fa-chart-line" },
                    new() { Title = "Average Order Value", Value = allOrders.Count == 0 ? "$0" : (deliveredOrders.Sum(x => x.TotalPrice) / Math.Max(1, deliveredOrders.Count)).ToString("C0"), SubText = "Across all completed orders", IconClass = "fa-solid fa-receipt" },
                    new() { Title = "Completion Rate", Value = allOrders.Count == 0 ? "0%" : $"{Math.Round((deliveredOrders.Count * 100m) / allOrders.Count, 1)}%", SubText = "Orders completed on time", IconClass = "fa-solid fa-circle-check" },
                },
                OrdersPerDayChart = new AdminChartDto
                {
                    ChartId = "ordersPerDayChart",
                    Title = "Orders Per Day",
                    Labels = allOrders.GroupBy(o => o.CreatedAt.Date).OrderBy(g => g.Key).Select(g => g.Key.ToString("MM-dd")).ToList(),
                    Values = allOrders.GroupBy(o => o.CreatedAt.Date).OrderBy(g => g.Key).Select(g => (decimal)g.Count()).ToList(),
                },
                RevenueGrowthChart = new AdminChartDto
                {
                    ChartId = "revenueGrowthChart",
                    Title = "Revenue Growth",
                    Labels = deliveredOrders.GroupBy(o => new { o.CreatedAt.Year, o.CreatedAt.Month }).OrderBy(g => g.Key.Year).ThenBy(g => g.Key.Month).Select(g => $"{g.Key.Month}/{g.Key.Year}").ToList(),
                    Values = deliveredOrders.GroupBy(o => new { o.CreatedAt.Year, o.CreatedAt.Month }).OrderBy(g => g.Key.Year).ThenBy(g => g.Key.Month).Select(g => g.Sum(x => x.TotalPrice)).ToList(),
                },
                ...
                TopVendors = await _workshopRepository.GetAllAsNoTracking()
                    ...
                    .Take(5)
                    .ToListAsync(),
                TopCategories = await _productRepository.GetAllAsNoTracking()
        ...
        {
            var query = _productReportRepository.GetAllAsNoTracking().Include(r => r.Product).Include(r => r.User).OrderByDescending(r => r.Id);
            var reports = await query.Take(20).ToListAsync();

            return new AdminReportsPageDto
            {
                Filter = filter,
                Reports = reports.Select(r => new AdminReportListItemDto
                {
                    Id = r.Id,
        ...
        public Task<AdminSettingsDto> GetSettingsAsync()
        {
            return Task.FromResult(new AdminSettingsDto
            {
                ...
                {
                    new() { CreatedAt = DateTime.UtcNow.AddHours(-1), Action = "Settings Updated", PerformedBy = "Admin", Details = "Commission percentage changed." },
                    new() { CreatedAt = DateTime.UtcNow.AddDays(-1), Action = "Maintenance Mode", PerformedBy = "Admin", Details = "Maintenance mode disabled." },
                },
            });
        }
    }
