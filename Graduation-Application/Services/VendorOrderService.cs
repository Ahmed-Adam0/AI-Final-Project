...
namespace Graduation_Application.Services
{
    public class VendorOrderService : IVendorOrderService
    {
        ...
        )> GetVendorOrdersAsync(int workshopId, VendorOrdersFilterDto filter)
        {
            ...
            var orders = await query
                ...
                .Select(vo => new VendorOrderDashboardDto
                {
                    ...
                    CustomerName =
                        string.IsNullOrWhiteSpace(vo.MasterOrder.FirstName)
                        && string.IsNullOrWhiteSpace(vo.MasterOrder.LastName)
                            ? (
                                vo.MasterOrder.User != null
                                    ? vo.MasterOrder.User.FullName
                                    : "Customer"
                            )
                    ...
                    CustomerPhone =
                        (
                            ...
                            ? (
                                ...
                                ?? (
                                    vo.MasterOrder.User != null
                                        ? vo.MasterOrder.User.PhoneNumber
                                        : ""
                                )
                                ?? ""
                            )
                        ...
                        .Items.Select(oi => new VendorOrderItemDto
                        {
                            ...
                            ProductImage =
                                oi.Product != null && oi.Product.Images != null
                                    ? oi
                                        .Product.Images.OrderByDescending(img => img.IsPrimary)
        ...
        {
            if (dto.EstimatedDeliveryDate.Date < DateTime.UtcNow.Date)
            {
                throw new Exception(
