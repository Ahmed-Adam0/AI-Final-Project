...
namespace Graduation_Application.IServices.Admin
{
    ...
    {
        Task<AdminVendorsPageDto> GetVendorsPageAsync(AdminVendorsFilterDto filter, bool pendingOnly = false);
        ...
        Task RejectVendorAsync(int workshopId, string performedByAdminId, string rejectionReason, string? notes = null);
        Task ActivateVendorAsync(int workshopId, string performedByAdminId, string? notes = null);
        Task SuspendVendorAsync(int workshopId, string performedByAdminId, string reason, string? notes = null);
