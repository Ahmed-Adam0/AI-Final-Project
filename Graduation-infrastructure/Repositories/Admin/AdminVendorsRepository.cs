...
namespace Graduation_infrastructure.Repositories.Admin
{
    public class AdminVendorsRepository : IAdminVendorsRepository
    {
        ...
        {
            var workshops = _db.Workshops.AsNoTracking();

            var total = await workshops.CountAsync();
            var pending = await workshops.CountAsync(w => w.VerificationStatus == VendorVerificationStatus.Pending);
            var approved = await workshops.CountAsync(w => w.VerificationStatus == VendorVerificationStatus.Approved);
            var rejected = await workshops.CountAsync(w => w.VerificationStatus == VendorVerificationStatus.Rejected);
            var active = await workshops.CountAsync(w => w.AccountStatus == VendorAccountStatus.Active);
            var suspended = await workshops.CountAsync(w => w.AccountStatus == VendorAccountStatus.Suspended);
        ...
        public async Task<AdminVendorsPageDto> GetVendorsPageAsync(AdminVendorsFilterDto filter, bool pendingOnly)
        {
            ...
            query = query.Select(w => new Workshop
            {
                ...
                VerificationStatus = w.VerificationStatus == 0 && w.IsVerified
            ...
            if (pendingOnly)
            {
                query = query.Where(w => w.VerificationStatus == VendorVerificationStatus.Pending);
            }
            ...
            if (!string.IsNullOrWhiteSpace(filter.VerificationStatus)
                && Enum.TryParse<VendorVerificationStatus>(filter.VerificationStatus, true, out var verification))
            {
                query = query.Where(w => w.VerificationStatus == verification);
            }

            if (!string.IsNullOrWhiteSpace(filter.AccountStatus)
            ...
            return new AdminVendorsPageDto
            {
                    ...
                    new List<Graduation_Application.DTOs.Admin.AdminDashboardDTO.AdminStatusOptionDto>
                {
                    ...
                    new() { Value = VendorVerificationStatus.Pending.ToString(), Label = "Pending" },
                    new() { Value = VendorVerificationStatus.Approved.ToString(), Label = "Approved" },
                    new() { Value = VendorVerificationStatus.Rejected.ToString(), Label = "Rejected" },
                },
                AccountStatusOptions =
                    new List<Graduation_Application.DTOs.Admin.AdminDashboardDTO.AdminStatusOptionDto>
                {
                    new() { Value = string.Empty, Label = "All Account Statuses" },
                    new() { Value = VendorAccountStatus.Active.ToString(), Label = "Active" },
                    new() { Value = VendorAccountStatus.Suspended.ToString(), Label = "Suspended" },
                },
            };
        }
        ...
        public async Task<AdminVendorDetailsDto?> GetVendorDetailsAsync(int workshopId)
        {
            ...
            var verificationStatus = workshop.VerificationStatus == 0 && workshop.IsVerified
                ? VendorVerificationStatus.Approved
            ...
            var deliveredRevenue = await ordersQuery.Where(o => o.Status == "Delivered").SumAsync(o => (decimal?)o.TotalPrice) ?? 0m;

            var verificationHistory = await _db.VendorVerificationHistory
                .AsNoTracking()
                ...
                .Select(h => new AdminVendorVerificationHistoryItemDto
                {
                    ...
                    AdminName = h.PerformedByAdmin != null ? h.PerformedByAdmin.FullName : h.PerformedByAdminId,
                    Notes = h.Notes,
                    RejectionReason = h.RejectionReason,
                })
                .ToListAsync();

            var accountHistory = await _db.VendorAccountStatusHistory
                .AsNoTracking()
                .Where(h => h.WorkshopId == workshopId)
                .Include(h => h.PerformedByAdmin)
                .OrderByDescending(h => h.CreatedAt)
            ...
            return new AdminVendorDetailsDto
            {
                ...
                Verification = new AdminVendorVerificationDto
                {
                    ...
                    VerifiedByAdminName = workshop.VerifiedByAdmin != null ? workshop.VerifiedByAdmin.FullName : workshop.VerifiedByAdminId,
                ...
                Account = new AdminVendorAccountDto
                {
                    ...
                    StatusChangedByAdminName = workshop.AccountStatusChangedByAdmin != null ? workshop.AccountStatusChangedByAdmin.FullName : workshop.AccountStatusChangedByAdminId,
                },
        ...
        public async Task<AdminVendorHistoryPageDto> GetHistoryAsync(int page, int pageSize)
        {
            ...
            var verificationQuery = _db.VendorVerificationHistory
                .AsNoTracking()
                ...
                .Select(h => new AdminVendorUnifiedHistoryItemDto
                {
                    ...
                    AdminName = h.PerformedByAdmin != null ? h.PerformedByAdmin.FullName : h.PerformedByAdminId,
                    Details = string.IsNullOrWhiteSpace(h.RejectionReason) ? h.Notes : $"Reason: {h.RejectionReason}",
                    BadgeClass = h.NewStatus == VendorVerificationStatus.Approved ? "bg-success"
                        : h.NewStatus == VendorVerificationStatus.Rejected ? "bg-danger"
                        : "bg-warning",
                });

            var accountQuery = _db.VendorAccountStatusHistory
                .AsNoTracking()
                .Include(h => h.Workshop)
                .Include(h => h.PerformedByAdmin)
                .Select(h => new AdminVendorUnifiedHistoryItemDto
                {
                    ...
                    BadgeClass = h.NewStatus == VendorAccountStatus.Active ? "bg-success" : "bg-danger",
                });
